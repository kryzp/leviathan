...
void init()
{
	g_tex = gfx::Texture::create("D:\\_PROJECTS\\leviathan\\testing\\res\\textures\\p0.png");
}

void render()
{
	App::clear(0x0099AAFF);
	{
		...
		g_batch.push_matrix(Mat3x2::create_transform(Vec2::zero(), 0.0f, Vec2::one(), Vec2::zero()));
...
// note doesnt work for now cba to finish this
...
void init()
{
	...
	g_fbf2          = gfx::Framebuffer::create(WINDOW_WIDTH, WINDOW_HEIGHT, fbf2_attachments, 2);
	g_fbf3V         = gfx::Framebuffer::create(WINDOW_WIDTH, WINDOW_HEIGHT);
...
void update()
{
	...
	g_velocity = (Input::mouse_position() - App::window_size()/2.0f - g_transform.position()) * 0.25f;
...
void render()
{
	...
	// render g_fbf to g_fbf2 with a crt shader and extract bright colours
	{
		...
		g_batch.push_matrix(Mat3x2::create_translation(-App::draw_size()/2.0f));

		g_batch.push_material(mat);
		{
			g_batch.peek_shader()->use().set("u_time", Time::milliseconds);
			g_batch.texture();
		}
		g_batch.pop_material();
	...
	// apply gaussian blur to g_fbf2's bright colours and render that to g_fbf3
	{
		...
		g_batch.push_matrix(Mat3x2::create_translation(-App::draw_size()/2.0f/2.0f/2.0f));
		g_fbf3Downscale->clear();
		g_batch.push_matrix(Mat3x2::create_scale(1 / BLUR_SCALE));
		{
			g_batch.texture();
		}
		g_batch.pop_matrix();
		...
		g_batch.push_matrix(Mat3x2::create_translation(-App::draw_size()/2.0f));
		g_batch.push_matrix(Mat3x2::create_scale(BLUR_SCALE));
		g_fbf3H->clear();
		{
	...
	// finally, render g_fbf2 and g_fbf3[VH] with bloom shader to the world
	{
		...
		mat.samplers = { gfx::TextureSampler::pixel(), gfx::TextureSampler::pixel() };
