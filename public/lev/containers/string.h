...
namespace lev
{
	...
	{
		LEV_ASSERT(m_length < (TSize - 1), "Length must not exceed maximum size"); // -1 for '\0'
	...
	{
		LEV_ASSERT(other.m_length < (TSize - 1), "Length must not exceed maximum size");
	...
	{
		LEV_ASSERT((m_length + str.m_length) < (TSize - 1), "Final length must not exceed maximum size");
